...
def main():
    ...
    records=d["records"]
    covered={(str(r.get("countryId")).lower(),r.get("category")) for r in records if r.get("status","ACTIVE")=="ACTIVE"}
    missing=[]
    for c in countries:
        cid=str(c["id"]).lower()
    ...
    print(f"BASELINE COVERAGE: {len(countries)*len(CATEGORIES)-len(missing)}/{len(countries)*len(CATEGORIES)} ({100*(len(countries)*len(CATEGORIES)-len(missing))/(len(countries)*len(CATEGORIES):.1f}%)")
    if missing:
